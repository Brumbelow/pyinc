...
internal review surfaced the consistency issues fixed below. Do not use the tag.
...
- Reentrant database calls refuse loudly instead of deadlocking or going stale.
- Every file-reading entry point is bounded; pipes and devices answer as absent.
...
- `Input`, `Query` and `NodeKey` keys and `Resource.label()` must be exactly `str`.
- `FileStatResource` delivers `FileStatSnapshot` in every mode, not a view or a `dict`.
- `Query.compare` is removed; the kernel compares snapshots under the query's policy.
- `Subscription.unsubscribe()`, every administrative or observational `Database` entry point, and every `Database` read from a resource hook or a thread spawned inside a query raise `ReentrantDatabaseError`.
- Every high-level integration entrypoint is outside-only: from a query body it raises `CompositionError`.
- A resource that mutates a list, dict or set it holds between reads is refused with `UnsupportedValueError`.
- An adapter payload holding a container inside a shared or cyclic value is refused at the freeze.
- `semantic_equal(1, 1.0)` and `semantic_equal(True, 1)` are `False`; `semantic_equal(nan, nan)` is `True`.
- Checkpoint manifest v8 and action ledger v3; v7 and earlier raise `CheckpointVersionError`, and a load into another mode `CheckpointModeError`.
- Reflective namespace reads and directly captured `functools.cache` callables are refused as captures, and every query identity moved, so a checkpoint written by an earlier build misses and re-executes.
- The semantic-versioning promise covers `pyinc` and `pyinc.integrations`; `pyinc_tools` and `pyinc_codegen` are unstable.

| Symbol or behaviour | Before | After | What to do |
|---|---|---|---|
| `Input(key)`, `@query(key=...)`, `Resource.label()` | any `str` subclass | exactly `str` | pass `member.value` or a plain string |
...
- A self-referential symbolic link and an embedded null character are refused
  as unsafe paths, a `PyIncError` as well as an `OSError`, by the resources,
  the actions and the stores alike.
...
- Observers fire only when a stored value moves, detach per registration, and
  never receive an event committed before they subscribed; persisting a
  snapshot verifies the stored bytes, `ArtifactStore.contains` has its
  documented default, and `InMemoryArtifactStore.keys()` is read-only.
- Collapsing `FrozenDict` keys and `FrozenSet` members are refused, policies
  receive detached operands, and `explain_query_captures` agrees with the kernel.
...
- `ResolvedPathResource`, `FileStatAdapter`, `BUILTIN_ADAPTERS`,
  `AdapterContractError`, `CheckpointModeError` and `CompositionError` are added.
...
- Query identity widened: wraps-decorated captures, a `Query` handle's own state
  and a rebound captured module attribute move it, and a mutated adapter
  configuration raises `AdapterContractError`.
- The ambient-read guard follows threads started inside a query, a query that
  catches a sub-query's exception is marked untracked rather than cached,
  `freeze` always returns an owned snapshot, TOML nesting is accepted to 200
  levels, and the development-status classifier is `4 - Beta`.
- The documentation is restructured: the README keeps the entry path, the
  kernel contract is cut to its rules and states its guarantee as conditional,
  the architecture page folds into the index and the FAQ, and the 2.x
  migration guide is removed.
