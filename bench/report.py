...
def render_markdown(csv_path: Path, metadata: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "",
        "These baselines do **not** provide identical correctness or dependency semantics; the comparison is informational only.",
        "",
        "- `pyinc_incremental` — dependency-aware early cutoff, ownership-tracked outputs, stale deletion, and tamper repair.",
        "- `fresh_full` — a brand-new cache-free `Database` recomputed from scratch (the correctness oracle).",
        "- `naive_cache` — a deliberately simple whole-input recompute with no dependency-aware cutoff, ownership, or stale deletion.",
        "- `joblib_memory` — argument-based memoization; marked `N/A` where it cannot represent file-tree generation.",
        "",
        "Every `pyinc_incremental` row passed a byte-for-byte correctness check against `fresh_full`.",
        "",
        "## Results",
        "",
    ]
