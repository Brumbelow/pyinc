...
(`4b5f392`); the second is the current one.

| Operation | Earlier build (`4b5f392`) | Current | Speedup |
|---|---|---|---|
| Initial workspace analysis | 232.99 s | 101.72 s | ~2.3x |
| Workspace re-analysis after a one-file edit | 160.311 s | 1.687 s | ~95x |
| Warm single-file re-analysis | 10.010 s | 0.273 s | ~37x |
...
| Metric | Value |
|---|---|
| Update wall time | 1.747 s |
| Queries executed | 282 |
| Results reused | 17,758 |
| Results backdated | 84 |
...
```mermaid
graph LR
    %% This diagram shows only the changed file's own re-derivation cascade: 6 of the update's 282 executed queries. The rest are per-request entrypoint work elsewhere in the workspace (workspace- and module-level aggregation over files other than this one).
    ...
    %% 13 more resolve_module_location(_pytest.warning_types) node(s) elided (3 of 16 non-executed ones shown as a representative sample); every executed node is shown.
    n0["sourcefile(src/_pytest/warning_types.py)"]:::reuse
    ...
    n8["module_symbol_table_payload(src/_pytest/warning_types.py)"]:::reuse
...
Second, this is 6 of the update's 282 executed queries. The other 276 are
...
The same reasoning is what makes the 95x row a fair number rather than a lucky
