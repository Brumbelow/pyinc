...
| Key limits | Every XML `DOCTYPE` and entity declaration is rejected with an `xml-parse-error` diagnostic, as is element nesting deeper than 2000 levels — the diagnostic names that limit, and no analysis raises `RecursionError`. DTD/XSD validation, external entities, XInclude, streaming APIs, and general XPath are not supported. Dot paths identify hierarchy but do not index repeated siblings. |
