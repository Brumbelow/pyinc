...
| Key limits | A non-object top level has no sections. JSONC, JSON5, schema validation, JSON Pointer/Path, and `$ref` resolution are out of scope. Duplicate keys and non-finite numeric constants are rejected rather than silently normalized. A document too deeply nested for CPython's JSON scanner reports the fixed text `JSON parsing exhausted the interpreter stack` under `json-decode-error`, so the cached payload does not depend on which frame ran out of budget; the depth at which that happens still follows the interpreter's recursion limit and so is not a fixed property of the document. |
