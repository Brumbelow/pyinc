...
from .models import SchemaAnalysis, _decode_model
from .schema import (
    _snake,
    definition_model,
    definition_names,
...
def generate(
    ...
    return generate_outputs.reconcile(db, os.fspath(schema_path), root=out_dir)
...
def schema_analysis(db: Database, schema_path: str | os.PathLike[str]) -> SchemaAnalysis:
    ...
    diagnostics = tuple(diag for model in models for diag in model.diagnostics)
